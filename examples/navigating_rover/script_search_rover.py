...
def line_dist(ind, show_plot=False, print_time=False):
    ...
    """
    starttime = time.time()
    mdl = Rover(p=RoverParam('turn', x_start=5.0, drive_modes=drive_modes))
...
def montecarlo(verbose=True, ngen=2, show_sol=True, weight=0.5,
               filename='outputs_script_search_rover/rslt_random.csv', formulation=1):
...
def ea(verbose=True, ngen=5, show_space_during_opt=False, show_sol=True, weight=0.5,
       filename='outputs_script_search_rover/rslt_ea.csv', formulation=1):
...
def ccea(verbose=True, ngen=2, show_space_during_opt=False, show_sol=True,
         weight=0.5, filename='outputs_script_search_rover/rslt_ccea.csv', formulation=1):
...
def visualizations(soln, method="EA", figsize=(4, 4), ax=False, legend=True,
    ...
    mdl_range = Rover(params=RoverParam(
...
mdl = mdl.new(sp={'end_time': end_time},
              p=RoverParam('turn', x_start=5.0, drive_modes=drive_modes),
              track=track)
