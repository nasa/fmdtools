...
# do nothing (default):
# params={'faultpolicy':{(a-1,b-1,c-1):(0,0) for a,b,c in np.ndindex((3,3,3))}}

# try to counteract leaks directly in valve:
# params={{(a-1,b-1,c-1):(-(a-1),-(c-1)) for a,b,c in np.ndindex((3,3,3))}}
# mdl = Tank(params={'capacity':20,'turnup':0.0,
#                   'faultpolicy': {(a-1, b-1, c-1): (-(a-1), -(c-1))
...
# result = minimize(x_to_descost, [1000, 0.5],
# method='trust-constr', bounds =((10, 100),(0,1)))

# xres=[0 for i in range(0,54)]
# cost = x_to_rcost2(xres)

# startpt = [(0,1) for i in range(0,27)]+[(10,100) for i in range(0,27)]
# result = differential_evolution(x_to_rcost2, startpt, maxiter=20, popsize=10)
