...
class UrbanGridParam(CoordsParam):
    """
    Define the grid parameters (beach is a 75-33 grid of 100m blocks, so 7.5km x 3.3km).
...
class StreetGrid(Coords):
    ...
    def init_properties(self, *args, **kwargs):
        land_points = [[float(x), float(y)] for x in range(0, 4750, 100) for y in range(0, 200, 100)]
...
def plot_env_with_traj(mdlhists, mdl, legend=True, title="UAS Water Rescue"):
    ...
    # add land/ocean to legend
    land_patch = mpatches.Patch(color="lemonchiffon", label="land")
    ocean_patch = mpatches.Patch(color="skyblue", label="ocean")
    handles, labels = ax.get_legend_handles_labels()
    ax.legend(handles=[land_patch, ocean_patch] + handles, labels=["land", "ocean"] + labels)
