...
class Avionics(FxnBlock):
    ...
    def dynamic_behavior(self, time):
        ...
        if self.m.in_mode("drive"):
            ...
            if in_area(*self.p.end, 1, *self.pos_signal.s.get("x", "y")):
                self.m.set_mode("finished")
            elif self.video.s.quality == 0:
                self.m.set_mode("em_off")
            elif not self.faultystates_in_bounds():
                self.m.set_mode("em_off")
            else:
                ycorrection = np.arctan(
                    (self.pos_signal.s.y - self.pos_signal.s.liney)
                    / (
                        self.pos_signal.s.vel
                        * np.cos(np.pi / 180 * self.pos_signal.s.heading)
                        + 0.001
                    )
                )
                xcorrection = np.arctan(
                    (self.pos_signal.s.x - self.pos_signal.s.linex)
                    / (
                        self.pos_signal.s.vel
                        * np.sin(np.pi / 180 * self.pos_signal.s.heading)
                        + 0.001
                    )
                )
                ...
                if self.video.s.quality == 0.5:
                    ang_diff = (
                        np.arctan(
                            (self.pos_signal.s.y - self.p.end[1])
                            / (self.pos_signal.s.x - self.p.end[0])
                        )
                        - self.pos_signal.s.heading
                        + turn_fault_correction
                    )
                else:
                    ang_diff = (
                        self.pos_signal.s.angle
                        - self.pos_signal.s.heading
                        + turn_fault_correction
                        - 5.5 * (xcorrection + ycorrection)
                    )
                rdiff = translate_angle(ang_diff) / 180
                vel_fault_correction = (
                    1
                    + self.p.cor_f * (self.faultystates.s.friction)
                    + self.p.cor_t * (self.faultystates.s.transfer - 1)
                )
                vel_adj = max(0.2, 1 - 0.9 * abs(rdiff * 20)) * vel_fault_correction
                self.control.s.put(
                    rpower=vel_adj * (1 + (rdiff)), lpower=vel_adj * (1 - (rdiff))
                )
    ...
    def faultystates_in_bounds(self):
        return (
            self.p.lb_f <= self.faultystates.s.friction <= self.p.ub_f
            and self.p.lb_d <= self.faultystates.s.drift <= self.p.ub_d
            and self.p.lb_t <= self.faultystates.s.transfer <= self.p.ub_t
        )
...
class Drive(FxnBlock):
    ...
    def dynamic_behavior(self, time):
        ...
        rpower = (
            self.m.s.transfer * self.ee_in.s.v * self.motor_control.s.rpower / 15
            + self.m.s.drift
        )
        lpower = (
            self.m.s.transfer * self.ee_in.s.v * self.motor_control.s.lpower / 15
            - self.m.s.drift
        )
        ...
        if (lpower + rpower) > 100:
            ...
            self.ground.s.inc(
                ang=180
                / np.pi
                * np.arctan((rpower - lpower) / (rpower + lpower + 0.001))
            )
            self.ground.s.ang = translate_angle(self.ground.s.ang)
            self.ground.s.inc(
                x=np.cos(np.pi / 180 * self.ground.s.ang) * self.ground.s.vel,
...
class Perception(FxnBlock):
    ...
    def dynamic_behavior(self, time):
        if self.m.in_mode("off"):
            ...
            if self.ee.s.v > 8:
                if in_area(
                    *self.ground.s.get("linex", "liney"),
                    self.rad,
                    *self.ground.s.get("x", "y")
                ):
...
class Power(FxnBlock):
    ...
    def static_behavior(self, time):
        ...
        if self.m.in_mode("charge"):
            ...
            self.s.power = (
                1.0
                + self.ee_12.s.mul("v", "a")
                + self.ee_5.s.mul("v", "a")
                + self.ee_15.s.mul("v", "a")
            )
...
class Communications(FxnBlock):
    ...
    def dynamic_behavior(self, time):
        if self.ee_12.s.v == 12:
            ...
            self.comms.s.put(x=0, y=0, vel=0, heading=0)
    ...
    def dynamic_behavior(self, t):
        if self.p.linetype == "sine":
            self.ground.s.angle = sin_angle_func(
                self.ground.s.x, self.p.amp, self.p.period
            )
            self.ground.s.linex, self.ground.s.liney = sin_func(
                self.ground.s.x, self.ground.s.y, self.p.amp, self.p.period
            )
        elif self.p.linetype == "turn":
            self.ground.s.angle = turn_angle_func(
                self.ground.s.x, self.p.radius, self.p.start
            )
            self.ground.s.linex, self.ground.s.liney = turn_func(
                self.ground.s.x, self.ground.s.y, self.p.radius, self.p.start
            )
        self.ground.s.lbx = self.ground.s.linex + 1.5 * np.sin(
            self.ground.s.angle * np.pi / 180
        )
        self.ground.s.lby = self.ground.s.liney - 1.5 * np.cos(
            self.ground.s.angle * np.pi / 180
        )
        self.ground.s.ubx = self.ground.s.linex - 1.5 * np.sin(
            self.ground.s.angle * np.pi / 180
        )
...
def in_bounds(x, y, lx, ly, ux, uy, tol=0.001):
    l_slope = ly / (lx + 0.000001)
    u_slope = uy / (ux + 0.000001)
    pt_slope = y / (x + 0.000001)
    if l_slope <= pt_slope <= u_slope:
        return True
    elif u_slope - l_slope < tol:
        if l_slope - tol <= pt_slope <= u_slope + tol:
            return True
        else:
            return False
    else:
...
def turn_angle_func(x, radius, start):
    if list in {type(x), type(radius), type(start)}:
        raise Exception(
            "Invalid x,radius,start: " + str(x) + "," + str(radius) + "," + str(start)
        )
    if x >= start + radius:
        return 90.0
    elif x >= start:
        return (
            90 - np.arccos(((x - start) / radius)) * 180 / np.pi
...
class Rover(Model):
    ...
    def indicate_finished(self, time):
        if (
            ...
            or (time > 5 and self.fxns["avionics"].m.in_mode("standby"))
            or self.fxns["avionics"].m.in_mode("em_off", "finished")
        ):
            return True
        else:
            return False
    ...
    def find_classification(self, scen, mdlhist):
        ...
        f_t = min(
            len(mdlhist.faulty.flows.ground.s.x), len(mdlhist.nominal.flows.ground.s.y)
        )

        tot_deviation = np.sum(
            np.sqrt(
                (
                    mdlhist.nominal.flows.ground.s.x[:f_t]
                    - mdlhist.faulty.flows.ground.s.x[:f_t]
                )
                ** 2
                + (
                    mdlhist.nominal.flows.ground.s.y[:f_t]
                    - mdlhist.faulty.flows.ground.s.y[:f_t]
                )
                ** 2
            )
        )
...
def plot_course(hist, label=True, ax=False):
    ...
    if label == True:
        nom_lab = "Nominal"
        bound_lab = "Bounds"
        center_lab = "Center-line"
    else:
        nom_lab = "_nolegend_"
        bound_lab = "_nolegend_"
        center_lab = "_nolegend_"

    ax.plot(hist.flows.ground.s.x, hist.flows.ground.s.y, color="blue")
    ax.scatter(
        hist.flows.ground.s.x[-1],
        hist.flows.ground.s.y[-1],
        color="blue",
        marker="*",
        label=nom_lab,
    )
...
def plot_trajectories(
    ...
    for mode, hist in range_hist.items():
        if mode[6:11] == "hmode":
            ax.plot(
                hist.flows.ground.s.x,
                hist.flows.ground.s.y,
                color="yellow",
                alpha=rangealpha,
            )
            ax.scatter(
    ...
    for mode, hist in mdlhists.items():
        if mode == "nominal":
            ...
            if show_labels:
                ...
                ax.annotate(
                    label,
                    xy=(hist.flows.ground.s.x[-1], hist.flows.ground.s.y[-1]),
                    fontsize=8,
                    xytext=(textoffset * np.sin(randang), textoffset * np.cos(randang)),
                    textcoords="offset points",
                )
    if app:
        ax.scatter(
            hist.flows.ground.s.x[int(app.times[0]) - 1],
            hist.flows.ground.s.y[int(app.times[0]) - 1],
            color="black",
            marker="X",
            s=5,
            label="fault time",
        )
    if not in_mdlhists and nomhist:
        plot_course(nomhist, ax=ax)
    if xlim:
        ax.set_xlim(*xlim)
    if ylim:
        ax.set_ylim(*ylim)
...
def compare_trajectories(
    mdlhist1,
    mdlhist2,
    mdlhist1_name="fault trajectories",
    mdlhist2_name="comparison trajectories",
    faulttimes=[],
    nomhist=[],
):
    ...
    if faulttimes:
        xfaults = [
            x
            for ind, x in enumerate(nomhist.flows.ground.s.x)
            if ind + 1 < len(nomhist.time) and nomhist.time[ind + 1] in faulttimes
        ]
        yfaults = [
            y
            for ind, y in enumerate(nomhist.flows.ground.s.y)
            if ind + 1 < len(nomhist.time) and nomhist.time[ind + 1] in faulttimes
        ]
        plt.scatter(
            xfaults, yfaults, marker="x", color="black", label="fault times", zorder=3
        )
    if nomhist:
        plot_course(nomhist)
...
def plot_map(mdl, mdlhist):
    ...
    plt.scatter(
        mdl.p.end[0], mdl.p.end[1], label="End Location", marker="X", color="grey"
    )
...
def plot_centerline_err(mdl, mdlhist):
    ...
    if mdl.p["linetype"] == "sine":
        y_line = [
            sin_func(x, y_rover[i], mdl.p["amp"], mdl.p["period"])[1]
            for i, x in enumerate(x_rover)
        ]
    elif mdl.p["linetype"] == "turn":
        y_line = [
            turn_func(x, y_rover[i], mdl.p["radius"], mdl.p["start"])[1]
            for i, x in enumerate(x_rover)
        ]
...
class_tree = {
    "rover": [-0.07367185100835244, 0.6410710936138487],
...
pos_bip = {
    "power": [-0.684772948203272, -0.2551613615446115],
