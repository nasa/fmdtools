...
class Battery(Component):
    ...
    def behavior(self, FS, EEoutr, time):
        ...
        if time > self.time:
            ...
            self.time=time
        if self.has_fault('nocharge'):    self.soc, self.Et, Er_res = 0.0,0.0, EEoutr
...
class Line(Component):
    ...
    def behavior(self, EEin, Ctlin, cmds, Force):
        ...
        if self.has_fault('short'):                   self.Eti, self.Eto = 0.0, np.inf
        elif self.has_fault('openc'):                 self.Eti, self.Eto =0.0, 0.0
        ...
        elif self.has_fault('mechfriction'):          self.Mt, self.Eti = 0.5, 2.0
        if self.has_fault('stuck'):                   self.Pt, self.Mt, self.Eti = 0.0, 0.0, 4.0
...
def plan_flight(elev):
    ...
    # x,y, elev
    startpt = [sq[0][0]+width/2, sq[0][1]+height/2, elev]
    endpt = [sq[1][0]-width/2, sq[1][1]+height/2, elev]
